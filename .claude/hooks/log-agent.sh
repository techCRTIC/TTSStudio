#!/bin/bash
# Claude Code SubagentStart hook — Personal AI Dev Studio
# Logs every subagent invocation to an audit trail.
# The matching SubagentStop hook (log-agent-stop.sh) closes each entry.
...
echo "[$TIMESTAMP] SUBAGENT_START agent=$AGENT_TYPE" >> "$LOG_FILE"
